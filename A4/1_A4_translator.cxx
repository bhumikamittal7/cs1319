...
int sizeOfType(symbolType* t){
    ...
    else if(t->type.compare("char")==0) return size_of_char;
    else if(t->type.compare("int")==0) return size_of_int;
    else if(t->type.compare("arr")==0) return t->width * sizeOfType(t->arrtype);
    else if(t->type.compare("ptr")==0) return size_of_pointer;
    else if(t->type.compare("func")==0) return bType.size[6];
    else return -1;
}

string printType(symbolType* t){
    if(t==NULL) return bType.type[0];
    else if(t->type.compare("void")==0) return bType.type[1];
    else if(t->type.compare("char")==0) return bType.type[2];
    else if(t->type.compare("int")==0) return bType.type[3];
    else if(t->type.compare("ptr")==0) return bType.type[4] + "(" + printType(t->arrtype) + ")";
    else if(t->type.compare("arr")==0) return bType.type[5] + "(" + convertToString(t->width) + "," + printType(t->arrtype) + ")";
    else if(t->type.compare("func")==0) return bType.type[6];
