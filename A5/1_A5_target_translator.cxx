		...
		if(op=="param"){
			...
			if (op=="PLUS") {
				...
				if (flag) {
					...
					asmfile << "movl \t" << table->array[arg1] << "(%rbp), " << "%eax" << endl;
					asmfile << "\tmovl \t" << table->array[arg2] << "(%rbp), " << "%edx" << endl;
				...
				else asmfile << "\timull \t" << table->array[arg2] << "(%rbp), " << "%eax" << endl;
				asmfile << "\tmovl \t%eax, " << table->array[result] << "(%rbp)";			
			}
				...
				asmfile << "movq\t" << table->array[arg2] << "(%rbp), "<<"%rdx" << endl;
				asmfile << "\tmovq\t" << "%rdx, " << off << "(%rbp)";
			}	 
			else if (op=="RETURN") {
				if(result!="") asmfile << "movl\t" << table->array[result] << "(%rbp), "<<"%eax";
				else asmfile << "nop";
			}
			else if (op=="param") {
				params.push_back(result);
				...
				for (list <sym>::iterator it = t->table.begin(); it!=t->table.end(); it++) {
					...
					if (it->category== "param") {
						if(j==0) {
							...
							asmfile << "movl \t" << table->array[params[i]] << "(%rbp), " << "%eax" << endl;
							asmfile << "\tmovq \t" << table->array[params[i]] << "(%rbp), " << "%rdi" << endl;
							j++;
						}
						else if(j==1) {
							
							asmfile << "movl \t" << table->array[params[i]] << "(%rbp), " << "%eax" << endl;
							asmfile << "\tmovq \t" << table->array[params[i]] << "(%rbp), " << "%rsi" << endl;
							j++;
						}
						else if(j==2) {
							
							asmfile << "movl \t" << table->array[params[i]] << "(%rbp), " << "%eax" << endl;
							asmfile << "\tmovq \t" << table->array[params[i]] << "(%rbp), " << "%rdx" << endl;
							j++;
						}
						else if(j==3) {
							
							asmfile << "movl \t" << table->array[params[i]] << "(%rbp), " << "%eax" << endl;
							asmfile << "\tmovq \t" << table->array[params[i]] << "(%rbp), " << "%rcx" << endl;
							j++;
						}
						else {
							asmfile << "\tmovq \t" << table->array[params[i]] << "(%rbp), " << "%rdi" << endl;							
