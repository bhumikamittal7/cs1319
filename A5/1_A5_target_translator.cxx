	...
	for (vector<quad>::iterator it = Array.begin(); it!=Array.end(); it++) {
		...
		if(op=="PARAM"){
			...
			if (op=="ADD") {
				...
				if (flag) {
					sfile << "addl \t$" << atoi(arg2.c_str()) << ", " << table->array[arg1] << "(%rbp)";
				}
				else {
					sfile << "movl \t" << table->array[arg1] << "(%rbp), " << "%eax" << endl;
					sfile << "\tmovl \t" << table->array[arg2] << "(%rbp), " << "%edx" << endl;
					sfile << "\taddl \t%edx, %eax\n";
					sfile << "\tmovl \t%eax, " << table->array[result] << "(%rbp)";
				}
			}
				...
				else sfile << "\timull \t" << table->array[arg2] << "(%rbp), " << "%eax" << endl;
				sfile << "\tmovl \t%eax, " << table->array[result] << "(%rbp)";			
			}
				...
				sfile << "movl \t" << table->array[arg1] << "(%rbp), " << "%eax" << endl;
				sfile << "\tcltd" << endl;
				sfile << "\tidivl \t" << table->array[arg2] << "(%rbp)" << endl;
				sfile << "\tmovl \t%eax, " << table->array[result] << "(%rbp)";		
			}
				...
				sfile << "movq \t$.LC" << arg1 << ", " << table->array[result] << "(%rbp)";
			}
			else if (op=="EQUALCHAR")	{
				sfile << "movb\t$" << atoi(arg1.c_str()) << ", " << table->array[result] << "(%rbp)";
			}	

			
			else if (op=="EQOP") {
				sfile << "movl\t" << table->array[arg1] << "(%rbp), %eax\n";
				sfile << "\tcmpl\t" << table->array[arg2] << "(%rbp), %eax\n";
				...
				sfile << "leaq\t" << table->array[arg1] << "(%rbp), %rax\n";
				sfile << "\tmovq \t%rax, " <<  table->array[result] << "(%rbp)";
			}
			else if (op=="PTRR") {
				sfile << "movl\t" << table->array[arg1] << "(%rbp), %eax\n";
				sfile << "\tmovl\t(%eax),%eax\n";
				...
				sfile << "movl\t" << table->array[result] << "(%rbp), %eax\n";
				sfile << "\tmovl\t" << table->array[arg1] << "(%rbp), %edx\n";
				sfile << "\tmovl\t%edx, (%eax)";
			} 			
			else if (op=="UMINUS") {
				sfile << "negl\t" << table->array[arg1] << "(%rbp)";
			}
				...
				sfile << "movq\t" << table->array[arg2] << "(%rbp), "<<"%rdx" << endl;
				sfile << "\tmovq\t" << "%rdx, " << off << "(%rbp)";
			}	 
			else if (op=="RETURN") {
				if(result!="") sfile << "movl\t" << table->array[result] << "(%rbp), "<<"%eax";
				else sfile << "nop";
			}
			else if (op=="PARAM") {
				params.push_back(result);
				...
				for (list <sym>::iterator it = t->table.begin(); it!=t->table.end(); it++) {
					...
					if (it->category== "param") {
						if(j==0) {
							...
							sfile << "movl \t" << table->array[params[i]] << "(%rbp), " << "%eax" << endl;
							sfile << "\tmovq \t" << table->array[params[i]] << "(%rbp), " << "%rdi" << endl;
							j++;
						}
						else if(j==1) {
							
							sfile << "movl \t" << table->array[params[i]] << "(%rbp), " << "%eax" << endl;
							sfile << "\tmovq \t" << table->array[params[i]] << "(%rbp), " << "%rsi" << endl;
							j++;
						}
						else if(j==2) {
							
							sfile << "movl \t" << table->array[params[i]] << "(%rbp), " << "%eax" << endl;
							sfile << "\tmovq \t" << table->array[params[i]] << "(%rbp), " << "%rdx" << endl;
							j++;
						}
						else if(j==3) {
							
							sfile << "movl \t" << table->array[params[i]] << "(%rbp), " << "%eax" << endl;
							sfile << "\tmovq \t" << table->array[params[i]] << "(%rbp), " << "%rcx" << endl;
							j++;
						}
						else {
							sfile << "\tmovq \t" << table->array[params[i]] << "(%rbp), " << "%rdi" << endl;							
				...
				for (list <sym>::iterator it = t->table.begin(); it!=t->table.end(); it++) {
					if (it->category== "param") {
						if (i==0) {
							sfile << "\tmovq\t%rdi, " << table->array[it->name] << "(%rbp)";
							i++;
						}
						else if(i==1) {
